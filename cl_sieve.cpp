...
void write_state( searchData & sd )
{
	...
	if (fprintf(out,"%" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "\n",sd.workunit,sd.p,sd.primecount,sd.checksum,sd.factorcount) < 0){
		if (sd.write_state_a_next)
...
int read_state( searchData & sd )
{
	...
	else if (fscanf(in,"%" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "\n",&workunit_a,&current_a,&primecount_a,&checksum_a,&factorcount_a) != 5){
	...
	else if (fscanf(in,"%" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "\n",&workunit_b,&current_b,&primecount_b,&checksum_b,&factorcount_b) != 5){
                fprintf(stderr,"Cannot parse %s !!!\n",STATE_FILENAME_B);
                good_state_b = false;
        }
        else{
...
void getResults( progData pd, searchData & sd, sclHard hardware ){
	...
	uint64_t * h_checksum = (uint64_t *)malloc(2*sizeof(uint64_t));
	if( h_checksum == NULL ){
	...
	// blocking read
	sclRead(hardware, 2*sizeof(uint64_t), pd.d_checksum, h_checksum);

	sd.checksum += h_checksum[0];
	sd.primecount += h_checksum[1];
...
void cl_sieve( sclHard hardware, searchData & sd ){
	...
	pd.d_primecount = clCreateBuffer( hardware.context, CL_MEM_READ_WRITE, 2*sizeof(cl_uint), NULL, &err );
        ...
        pd.d_checksum = clCreateBuffer( hardware.context, CL_MEM_READ_WRITE, 2*sizeof(cl_ulong), NULL, &err );
        if ( err != CL_SUCCESS ) {
		fprintf(stderr, "ERROR: clCreateBuffer failure.\n");
                printf( "ERROR: clCreateBuffer failure.\n" );
		exit(EXIT_FAILURE);
	...
	if(debuginfo){
		printf("setup,sieve,check kernel global size: %" PRIu64 "\n", pd.sieve.global_size[0]);
	}
	...
	if(debuginfo){
		printf("kernel_nstep %u nstep %u mont_nstep %u bbits %i r0 %" PRIu64 " nmin %u bbits1 %i r1 %" PRIu64 "\n",sd.kernel_nstep,sd.nstep,sd.mont_nstep,sd.bbits,sd.r0,sd.nmin,sd.bbits1,sd.r1);
	}
...
//	-p 2563602e7 -P 2563619e7 -n 10000000 -N 25000000 -c		nstep 19
	sd.pmin = 25636020000000;
	sd.pmax = 25636190000000;
	sd.nmin = 10000000;
	sd.nmax = 25000000;
	sd.kmin = 0;
	sd.kmax = 0;
	...
	if( sd.factorcount == 5 && sd.primecount == 5503989 && (sd.checksum == 0x78EDCFFA97B14E0C) ){
...
//	-p 5564100e8 -P 5564400e8 -N 100000 -c
	sd.pmin = 556410000000000;
	sd.pmax = 556440000000000;
	sd.nmin = 0;
	sd.nmax = 100000;
	sd.kmin = 0;
	sd.kmax = 0;
	sd.cw = true;
	cl_sieve( hardware, sd );
	if( sd.factorcount == 1 && sd.primecount == 883589976 && (sd.checksum == 0x9AB585E778EC4D83) ){
...
//	-p838338346e9 -P838338348e9 -k5 -K9999 -n6000000 -N9000000	nstep 32
	sd.pmin = 838338346000000000;
	sd.pmax = 838338348000000000;
	sd.nmin = 6000000;
	sd.nmax = 9000000;
	sd.kmin = 5;
	sd.kmax = 9999;
	sd.cw = false;
	cl_sieve( hardware, sd );
	if( sd.factorcount == 1 && sd.primecount == 48457248 && (sd.checksum == 0xA97AB8FF171ECBE2) ){
...
//	-p42070e9 -P42072e9 -k 1201 -K 9999 -N 2000000			nstep 31
	sd.pmin = 42070000000000;
	sd.pmax = 42072000000000;
	sd.nmin = 0;
	sd.nmax = 2000000;
	sd.kmin = 1201;
	sd.kmax = 9999;
	sd.cw = false;
	cl_sieve( hardware, sd );
	if( sd.factorcount == 2917 && sd.primecount == 63756679 && (sd.checksum == 0x1BE7064083AA1EB1) ){
